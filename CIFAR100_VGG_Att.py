...
class SResNetTrainer(train_classify.Trainer):
    ...
    def load_model(self, args, num_classes):
        model = spiking_vgg.__dict__[args.model](pretrained=args.pretrained, spiking_neuron=neuron.LIFNode,
                                                        surrogate_function=surrogate.ATan(), detach_reset=True, num_classes=num_classes, att=True)
...
if __name__ == "__main__":
    ...
    sys.stdout = open('/userhome/ZRJ/ImageNet_Test/log/log.txt', 'w+')
    sys.stderr = open('/userhome/ZRJ/ImageNet_Test/log/errorlog.txt', 'w+')'''
